...
const PerfumeManage = ({ product, handleDelete }) => {
    ...
    return (
        <div className='container flex mb-3'>
            ...
            <Link to={`/myInventoryOrder/${_id}`} className='py-2 px-4 mx-2 bg-blue-500 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-75'>Order</Link>
        </div>
    );
};
